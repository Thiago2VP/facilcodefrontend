...
import { SectionText, TextTittle, QrDiv, QrMov, QrArea, QrAction, DownOptions, SavOptions } from './styled';
...
export default function Text({ match }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!text) {
      formErrors = true;
      toast.error('Texto precisa ser preenchido');
    }
  ...
  return (
    <Container>
      ...
      <SectionText>
        <div>
          <Form onSubmit={handleSubmit} onClick={updateForm}>
            <fieldset>
              ...
              <label htmlFor="text">
                Texto ou link:
                <input type="text" value={text} onChange={(e) => setText(e.target.value)} placeholder="Obrigatório" />
              </label>
            </fieldset>
            <fieldset>
        ...
        <QrDiv>
          <QrMov>
            <TextTittle>Pré Visualização</TextTittle>
            <QrArea>
              {qrSrc ? (
                <div>
